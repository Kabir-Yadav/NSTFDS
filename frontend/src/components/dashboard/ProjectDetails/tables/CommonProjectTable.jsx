...
const CommonProjectTable = ({
  ...
  const filterData = () => {
    ...
    // Apply PSU filter
    if (selectedpsu) {
      filtered = filtered.filter((item) => item.psu === selectedpsu);
    }
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text(`${projectName || "Project"} Report`, 14, 10);
    ...
    const tableColumn = showCategory
      ? [...baseColumns.slice(0, 6), "Category", ...baseColumns.slice(6)]
      : baseColumns;
    ...
    const tableRows = filteredData.map((row) => {
      const baseRow = [
        ...
        row.status,
        row.total_cost,
        row.proof_image_url ? "View" : "No Image",
      ];

      return showCategory
        ? [...baseRow.slice(0, 6), row.item_type, ...baseRow.slice(6)]
        : baseRow;
    });
    ...
    doc.autoTable({
      ...
      startY: 20,
      columnStyles: {
        [tableColumn.length - 1]: { cellWidth: 35 },
      },
      styles: {
        fontSize: 8,
      },
  ...
  const exportToCSV = () => {
    ...
    const headers = showCategory
      ? [...baseHeaders.slice(0, 6), "Category", ...baseHeaders.slice(6)]
      : baseHeaders;
    ...
    link.setAttribute("download", `${projectName || "Project"}Report.csv`);
...
export default CommonProjectTable;
